...
class Manager(object):
    ...
    STATUS_MAP = {
        ...
        "PROBLEM": hatohol_def.TRIGGER_STATUS_PROBLEM,
    }
    ...
    def __call__(self):
        ...
        while should_continue():
            ...
            if num_events == 0:
                break
            count += num_events
            self.__parseEvents(response["events"], offset)
            offset += self.MAX_EVENTS_CHUNK
        logger.info("Checked %d events" % count)

    def __parseEvents(self, events, offset):
        arg = {
        ...
        serial = offset - self.NUM_SELF_TRIGGER_EVENT + 1;
        for event in events:
            arg["serial"] = serial
            base_event = libchocoload.generate_event_std_elem(**arg)
    ...
    def __verifyEventCommon(self, base, actual):
        # TODO: check compare the serverId
        self.__assert_not_none(actual.get("serverId"))
    ...
    def __verifyLoadedEvent(self, base, actual):
        self.__verifyEventCommon(base, actual)
        self.__assert(actual["type"], self.TYPE_MAP[base["type"]])
        self.__assert(actual["status"], self.STATUS_MAP[base["status"]])
        self.__assert(actual["hostId"], base["hostId"])
        self.__assert(actual["triggerId"], base["triggerId"])
        self.__assert(actual["severity"], self.SEVERITY_MAP[base["severity"]])
